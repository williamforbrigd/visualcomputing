5// Animation Duration in seconds
...
// in the previous keyframes_alpha all the tangents were 0
...
function linearInterpolation(keyframes)
{
  ...
  // the equation is: y = (y_0 * (t_1 - t) + y_1 * (t - t_0))/(t_1 - t_0)
  // t is the normalizedTimeOffset
  // t_1 - t_0 is equal i 1 so don't need to divide with anything. 
  const res = prevKeyframeVal *(1.0 - normalizedTimeOffset) 
  + nextKeyframeVal * (normalizedTimeOffset);
  return res;
}
...
function cubicSplineInterpolation(keyframes)
{
  ...
  const keyframeDuration = keyframes[nextKeyframeIdx].time 
  - keyframes[prevKeyframeIdx].time;
  ...
  const t2 = t*t;
  const t3 = t*t*t;
  const res = (2*t3 - 3*t2 + 1)* prevKeyframeVal + 
              (t3 - 2*t2 + t) * prevTangent +
              (-2*t3 + 3*t2) * nextKeyframeVal +
              (t3 - t2) * nextTangent;
  return res;
}
